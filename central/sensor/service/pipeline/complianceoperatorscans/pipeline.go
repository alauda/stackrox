...
func (s *pipelineImpl) Reconcile(ctx context.Context, clusterID string, storeMap *reconciliation.StoreMap) error {
	...
	if features.ComplianceEnhancements.Enabled() {
		scanObjects, err := s.v2Datastore.GetScanObjectByCluster(ctx, clusterID)
		if err != nil {
			return err
		}
	...
	return reconciliation.Perform(store, existingIDs, "complianceoperatorscans", func(id string) error {
		if features.ComplianceEnhancements.Enabled() {
			if err := s.v2Datastore.DeleteScanObject(ctx, id); err != nil {
				return err
			}
		}
...
func (s *pipelineImpl) processV2ComplianceScan(ctx context.Context, event *central.SensorEvent, clusterID string) error {
	...
	case central.ResourceAction_REMOVE_RESOURCE:
		...
		// use V2 datastore
		return s.v2Datastore.DeleteScanObject(ctx, event.GetId())
	default:
		...
		return s.v2Datastore.UpsertScanObject(ctx, internaltov2storage.ComplianceOperatorScanObject(complianceScanObject, clusterID))
	}
}
