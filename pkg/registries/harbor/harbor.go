...
const (
	// GenericHarborRegistryType exposes the default registry type
	GenericHarborRegistryType = "harbor"
...
func Creator() (string, func(integration *storage.ImageIntegration) (types.Registry, error)) {
	return GenericHarborRegistryType, func(integration *storage.ImageIntegration) (types.Registry, error) {
		reg, err := NewHarborRegistry(integration, false)
		return reg, err
	}
}
...
func CreatorWithoutRepoList() (string, func(integration *storage.ImageIntegration) (types.Registry, error)) {
	return GenericHarborRegistryType, func(integration *storage.ImageIntegration) (types.Registry, error) {
		reg, err := NewHarborRegistry(integration, true)
		return reg, err
	}
}
...
func NewHarborRegistryWithConfig(cfg Config, integration *storage.ImageIntegration) (*Registry, error) {
	...
	scheme, err := urlfmt.GetRegistryScheme(endpoint)
	if err != nil {
		return nil, err
	}
...
// NewHarborRegistry creates a generic docker registry integration
func NewHarborRegistry(integration *storage.ImageIntegration, disableRepoList bool) (*Registry, error) {
	harborConfig, ok := integration.IntegrationConfig.(*storage.ImageIntegration_Harbor)
	if !ok {
		return nil, errors.New("Docker configuration required")
	}
	cfg := Config{
		Endpoint:        harborConfig.Harbor.GetEndpoint(),
		Username:        harborConfig.Harbor.GetUsername(),
		Password:        harborConfig.Harbor.GetPassword(),
		Insecure:        harborConfig.Harbor.GetInsecure(),
		DisableRepoList: disableRepoList,
	}
	return NewHarborRegistryWithConfig(cfg, integration)
}
...
func (r *Registry) Test() error {
	...
	if err != nil {
		log.Errorf("error testing harbor integration: %v", err)
